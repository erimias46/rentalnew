                ...
                <div class="card flex-grow" style="max-width: 420px">
                    ...
                    <form method="POST" class="p-6 grid grid-cols-1 gap-3" >
                        ...
                        <div>
                            ...
                            <input type="password" name="password" class="form-input" value="<?php echo $row['password'];?>" required>
                        </div>
...
if (isset($_POST['update_profile'])) {
    ...
    $profile_update = "UPDATE `user` SET `user_name`='$user_name', `password`='$password' WHERE `user_name` = '$user_name_0'"; 
    $result_update = mysqli_query($con, $profile_update);

    if ($result_update) {
        echo "<script>window.location = 'action.php?status=success&redirect=profile.php'; </script>";
    } else {
        echo "<script>window.location = 'action.php?status=error&redirect=profile.php'; </script>";
    }
}
