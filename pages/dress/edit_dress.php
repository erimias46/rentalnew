...
if (isset($_POST['update'])) {
    ...
    $id = $_GET['id']; // Assuming `dress_id` is passed for identifying the record
    $sql = "SELECT image FROM dress WHERE id='$id'";
    $result = mysqli_query($con, $sql);
    $row = mysqli_fetch_assoc($result);
    ...
    // Update the dress record with the new or old image
    $sql = "UPDATE dress SET dress_name='$dress_name', size='$size', type='$type', price='$price', quantity='$quantity', image='$image_path' WHERE id='$id'";
    $result = mysqli_query($con, $sql);

    if ($result) {
        echo "<script>window.location = 'action.php?status=success&redirect=all_dress.php';</script>";
        ...
        echo "<script>window.location = 'action.php?status=error&message=Error updating dress in the database.&redirect=all.php';</script>";
    }
}
                    ...
                    <div class="card">
                        ...
                        <div class="p-6">
                            ...
                            <form method="post" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                ...
                                <div class="mb-3">
                                    <label class="text-gray-800 text-sm font-medium inline-block mb-2">dress Name</label>
                                    <input type="text" name="dress_name"  class="form-input" list="dress_types" required  value="<?php echo $dress_name ?>">
                                    <datalist id="dress_types">
                                ...
                                <div class="mb-3">
                                    <div class="image-preview" id="imagePreview">
                                        ...
                                        <img id="previewImg" src="<?php echo !empty($image) ? $image : 'uploads/defaultdress.jpg'; ?>" alt="Product Image" width="200" />
                                    </div>
                                </div>
